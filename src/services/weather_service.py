...
class WeatherService:
    ...
    def format_current_weather(self, username: str, weather_data: dict) -> str:
        """Format current weather data as message."""
        answer = f"{username}, в <b>{weather_data['location_name']}</b>\n\n"
        if weather_data.get('state'):
            answer += f"\U0001F5FA <i>Регион:</i> <b>{weather_data['state']}</b>\n"
        if weather_data.get('country'):
            flag = self._country_flag(weather_data['country'])
            answer += f"{flag} <i>Код страны:</i> <b>{weather_data['country']}</b>\n"
        ...
        answer += f"\U0001F4CA <i>Давление:</i> <b>{weather_data['pressure']} мм</b>\n"
    ...
    def format_forecast(self, username: str, forecast_data: dict) -> str:
        """Format forecast data as message."""
        answer = f"{username}, в <b>{forecast_data['location_name']}</b>\n\n"
        if forecast_data.get('state'):
            answer += f"\U0001F5FA <i>Регион:</i> <b>{forecast_data['state']}</b>\n"
        if forecast_data.get('country'):
            flag = self._country_flag(forecast_data['country'])
            answer += f"{flag} <i>Код страны:</i> <b>{forecast_data['country']}</b>\n"
        ...
        for day in forecast_data['forecasts']:
            ...
            answer += f"\U0001F4CA <i>Давление:</i> <b>{day['pressure_avg']} мм</b>\n"
